...
const ShaderCanvas = () => {
  ...
  const [backgroundColor, setBackgroundColor] = useState(LIGHT_BG);
  ...
  return (
    <canvas
      ref={canvasRef}
      className="fixed top-0 left-0 w-full h-full block z-0 bg-[#F5F5F5]"
    />
  );
};
...
export const PricingCard = ({
  ...
  const cardClasses = `
    backdrop-blur-md bg-white border border-gray-200 text-black
    rounded-2xl shadow-md flex-1 max-w-xs px-7 py-8 flex flex-col transition-all duration-300
    ${isPopular ? "ring-2 ring-cyan-300 scale-105 shadow-lg" : ""}
  `;
  const buttonClasses = `
    mt-auto w-full py-2.5 rounded-xl font-semibold text-sm transition font-sans
    ${
      buttonVariant === "primary"
  ...
  return (
    <div className={cardClasses.trim()}>
      {isPopular && (
        <div className="absolute -top-4 right-4 px-3 py-1 text-xs font-semibold rounded-full bg-cyan-500 text-white">
          Most Popular
        </div>
      )}
      <div className="mb-3">
        <h2 className="text-3xl font-light tracking-tight text-black font-display">
          {planName}
        </h2>
        <p className="text-base text-gray-600 mt-1 font-sans">{description}</p>
      </div>
      <div className="my-6 flex items-baseline gap-2">
        <span className="text-4xl font-light text-black font-display">
          ${price}
        </span>
        <span className="text-sm text-gray-600 font-sans">/mo</span>
      </div>
    ...
    <div className="bg-[#F5F5F5] text-black min-h-screen w-full overflow-x-hidden">
      ...
      <main className="relative w-full min-h-screen flex flex-col items-center justify-center px-4 py-8">
        <div className="w-full max-w-5xl mx-auto text-center mb-14">
          ...
          <p className="mt-3 text-lg md:text-xl text-gray-700 max-w-2xl mx-auto font-sans">
            {subtitle}
          </p>
        </div>
